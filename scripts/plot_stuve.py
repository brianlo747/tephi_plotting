...
from plots.radiosonde.stuve.stuve_main import Stuve


def plot_wyoming_stuve(date, station, output_dir, theta_w=False):
    ...
    stu.plot_profile(sonde_data['pressure'].values, sonde_data['temperature'].values,
                     label='Temperature', color='red', linewidth=0.8)
    stu.plot_profile(sonde_data['pressure'].values, sonde_data['dewpoint'].values,
    ...
    #     stu.plot_profile(sonde_data['pressure'].values, wet_bulb - 273.15,
    #                      label='Wet Bulb', color='violet', linewidth=0.8)
    #
    stu.plot_barbs(pressures_winds['pressure'].values, pressures_winds['speed'].values,
                   pressures_winds['direction'].values + 180.0)
    stu.plot_main_title(sonde_metadata)
    stu.read_metadata(sonde_metadata)
    stu.save_tephi(output_dir=output_dir)
    # stu.save_tephi_manual(output_path=output_dir)
    plt.close('all')
...
if __name__ == '__main__':
    ...
    station_list = ['03882']
    output_dir = '/Users/brianlo/Desktop/Reading/PhD/WCD/output/stuve'
    ############################

    for station_num in station_list:
        try:
            plot_wyoming_stuve(date=main_date,
                               station=station_num,
